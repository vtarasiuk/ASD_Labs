...
{
	srand(seed);
	int** matrix = getMMemory(vertices, vertices);
	...
	Point* center)
{
	...
	graph->vertexRadius = vertexRadius;
	graph->power = verticesAmount;
	graph->center = center;
	Point** positions = getCirculesPositions(center, verticesAmount, vertexRadius, padding);
	graph->vertices = (Vertex*)malloc(verticesAmount * sizeof(Vertex));
