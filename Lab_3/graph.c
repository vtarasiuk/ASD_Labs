	...
	Point* c)
{
	...
	graph->matrix = M;
	graph->vertexRadius = v_radius;
	graph->strength = strength;
	graph->center = c;
	Point** positions = getVPos(c, strength, v_radius, space);
	graph->vertices = (Vertex*)malloc(strength * sizeof(Vertex));
...
void drawGraph(HDC hdc, HPEN pen,
	...
	int oriented)
{
	for (int i = 0; i < graph->strength; i++)
	{
	...
	for (int i = 0; i < graph->strength; i++)
	{
		...
		{
			int dist = abs(i - j);
			if (dist > graph->strength / 2)
			{
				dist = graph->strength / 2 - (dist - graph->strength / 2);
			}
			int sideLength = graph->strength / 3 + 1;
...
Point** _getTrianglePoints(int n, double side, Point* center)
{
...
{
	return _getTrianglePoints(circulesAmount,
...
int** makeSymmetricM(int** M, int v)
{
	int** matrix = getEmptyArray(v, v);
	for (int i = 0; i < v; i++)
	{
